...
- When opening a PR from the SSH/control-node checkout, first try the GitHub connector if it is available. If it fails with `Resource not accessible by integration`, fall back to `gh pr create` from the already-pushed branch with explicit `--base`, `--head`, `--title`, `--body`, and `--draft`. If `gh pr edit` fails on GitHub GraphQL project-card fields, update the PR body with the REST path instead, for example `gh api repos/<owner>/<repo>/pulls/<number> -X PATCH -f body=...`.
