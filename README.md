...
The first implementation now lives at `services/registry.yaml` and is exposed through the read-only `service_list` and `service_get` tools. `service_list` returns a compact declared inventory for low-context discovery, while `service_get` returns the full declared entry for one service. Both stay offline and avoid calling Kubernetes, Ansible, SSH, or any live host APIs.
