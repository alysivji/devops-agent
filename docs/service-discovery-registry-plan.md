...
- The service registry is static, curated by the repo, service-specific, safe
  to read offline, and focused on identity, ownership, and access paths.
...
The service registry is a narrow discovery source, not a general memory system.
It should answer questions such as "what service is this?", "where is it
supposed to live?", and "which automation owns it?" It is not the place to
store learned observations, prior decisions, operator notes, or workflow
history.

If the agent grows a broader memory capability later, that memory should be
separate from the service registry:

- Memory is broader than services and may include session-, workflow-, or
...
- Memory is not the source of truth for service identity or declared access
  paths.
...
- Remain registry-shaped: return declared service entries only, with no learned
  state, summaries, or inferred history mixed into the result.

No runtime API change is needed in this doc-only pass. If broader contextual
recall is added later, keep the boundary clean: use `service_*` tools for static
service discovery and `memory_*` tools or storage for general recall.
